...
def estimate_tfs_parameters_local(
    ...
    try:
        ...
        ae_1 = estimate_p_norm(x = df_fts["cumvariance_log_return"].values, y = estimated_1, p = p_norm)
        ae_2 = estimate_p_norm(x = df_fts["cumvariance_absolute_log_return"].values, y = estimated_2, p = p_norm)
        ae_3 = estimate_p_norm(x = df_fts["cumvariance_log_volatility"].values, y = estimated_3, p = p_norm)
        
        # Estimation of R squared (TFS) ----
        r2_1 = estimate_coefficient_of_determination(y = df_fts["cumvariance_log_return"].values, y_fitted = estimated_1)
        r2_2 = estimate_coefficient_of_determination(y = df_fts["cumvariance_absolute_log_return"].values, y_fitted = estimated_2)
        r2_3 = estimate_coefficient_of_determination(y = df_fts["cumvariance_log_volatility"].values, y_fitted = estimated_3)
